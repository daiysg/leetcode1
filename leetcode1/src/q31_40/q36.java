	...
	public boolean isValidSudoku(char[][] board) {
		...
		for (int i = 0; i < count; i++) {
			...
			for (int j = 0; j < count; j++) {
				...
				int mtxRowIdx = 3 * (i / 3);
				int mtxColIdx = 3 * (i % 3);
				int matrixNum = board[mtxRowIdx + j / 3][mtxColIdx + j % 3] == '.' ? -1
						: board[mtxRowIdx + j / 3][mtxColIdx + j % 3] - '0';
				if (rowNum > 0 && rowExist[rowNum] || colNum > 0
						&& colExist[colNum] || matrixNum > 0
						&& matrixExist[matrixNum]) {
					return false;
				}
				if (rowNum > 0)
